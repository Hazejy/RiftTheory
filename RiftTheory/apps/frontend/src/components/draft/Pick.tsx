import { Icon, lockOpen, lockClosed } from "../icons/RiftIcons";
...
export function Pick(props: Props) {
    ...
    return (
        <div
            data-pick-slot={pickLabel(props.team, props.index)}
            data-champion-key={pick().championKey ?? ""}
            aria-label={`${pickLabel(props.team, props.index)}: ${champion() ? championName(champion()!, config) : t("nextPick")}`}
        ...
        >
            ...
            <Show when={champion()}>
                <>
                    ...
                    <span class="absolute top-2 left-2 right-2 uppercase text-2xl leading-none">
                        <span class="block h-6 text-sm text-neutral-300">
                            {pickLabel(props.team, props.index)}
                        </span>
                        <span
                            class="block truncate"
                            title={championName(champion()!, config)}
                        >
            ...
            <PickResetButton team={props.team} index={props.index} />
