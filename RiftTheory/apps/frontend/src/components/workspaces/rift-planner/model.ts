...
export function emptyPlannerDocument(): PlannerDocument {
    return { ...EMPTY_BOARD, notes: "", customRoster: [], championSize: 38 };
}
        ...
        return {
            ...
            customRoster:
                "customRoster" in parsed && Array.isArray(parsed.customRoster)
                    ...
                    : [],
            championSize:
                "championSize" in parsed &&
                typeof parsed.championSize === "number"
                    ? Math.max(24, Math.min(64, parsed.championSize))
                    : 38,
        };
    } catch {
        return emptyPlannerDocument();
