        ...
        return new Map(
            suggestions.map((suggestion) => {
                ...
                const flexOptions = latestRoleEvidence(champion)
                    ...
                    .flatMap((role) => {
                        if (role.tier === "insufficient" || role.tier === "emerging" || observed?.tier === "insufficient" || observed?.tier === "emerging" || !observed) return [];
                ...
                return [
                    ...
                    ),
                    {
                        ...
                        strategicIdentity: colorEvidence
                            ? {
                                  mainColors: colorEvidence.profile.colors
                                      .filter(
                                          (color) =>
                                              color.assignment === "main",
                                      )
                                      .map((color) => color.color),
                                  offColors: colorEvidence.profile.colors
                                      .filter(
                                          (color) => color.assignment === "off",
                                      )
                                      .map((color) => color.color),
                                  reasoning: colorEvidence.profile.reasoning,
                                  reviewStatus:
                                      colorEvidence.profile.review_status,
                                  scope: colorEvidence.scope,
