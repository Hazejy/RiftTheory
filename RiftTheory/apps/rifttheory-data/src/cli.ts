...
import {
  DATABASE_PATH,
  HISTORICAL_COLOR_BASELINES_PATH,
  INTERACTION_EVIDENCE_PATH,
  PROVISIONAL_COLOR_BASELINES_PATH,
} from "./paths";
...
try {
  ...
  switch (command) {
    ...
    case "refresh": {
      ...
      const baselines = await importColorBaselines(
        database,
        HISTORICAL_COLOR_BASELINES_PATH,
        PROVISIONAL_COLOR_BASELINES_PATH,
      );
      ...
      console.log(
        `Updated ${result.champions} champions from ${status.publishedPatch ?? "no published patch"} to ${riot.patch} with ${records} curated records, ${baselines.historicalChampions} historical and ${baselines.provisionalChampions} provisional color baselines, plus ${interactions.records} interaction records`,
      );
      break;
    }
    case "import-curated": {
    ...
    case "import-color-baselines": {
      const result = await importColorBaselines(
        database,
        HISTORICAL_COLOR_BASELINES_PATH,
        PROVISIONAL_COLOR_BASELINES_PATH,
      );
      console.log(
        `Imported ${result.historicalChampions} historical and ${result.provisionalChampions} provisional color baselines (${result.records} assignments)`,
      );
      break;
    }
    case "import-roles": {
    ...
    case "build": {
      ...
      verifyDatabase(database);
      console.log(
        `Built ${result.champions} champions with ${records} curated records, ${baselines.historicalChampions} historical and ${baselines.provisionalChampions} provisional color baselines, plus ${interactions.records} interaction records${offline ? " (offline)" : ""}`,
      );
      break;
    }
    case "status":
