        ...
        return Object.entries(data.championData)
            .filter(([key, champion]) => {
                ...
                if (role !== undefined) {
                    ...
                    const roleGames = champion.statsByRole[role]?.games ?? 0;
                    const totalGames = ROLES.reduce<number>(
                        (total, candidateRole) =>
                            total +
                            (champion.statsByRole[candidateRole]?.games ?? 0),
                        0,
                    );
                    if (
                        roleGames < 500 ||
                    ...
                    <div class="flex flex-wrap gap-2">
                        <For each={props.document.players}>
                            {(player) => (
                                <span
                                    draggable={true}
                                    class="flex cursor-grab items-center gap-2 rounded-lg border border-neutral-700 bg-canvas px-3 py-2 text-sm"
                                    onDragStart={(event) =>
                                        event.dataTransfer?.setData(
                                            "application/x-rifttheory-player",
                                            JSON.stringify({
                                                playerId: player.id,
                                            }),
                                        )
